// Script to generate audio files for TOEFL ITP Package B Listening Section
// This script calls the local API route to generate audio
// Run with: bun run scripts/generate-audio-via-api.ts
...
// Package B Listening Questions - extracted from package-b.ts
const packageBListening = [
  ...
  // PART B - Longer Conversations (8 questions - 2 conversations)
  ...
  // PART C - Academic Talks (12 questions - 3 talks)
...
async function generateAudioViaAPI() {
  ...
  console.log(`Generating audio for ${packageBListening.length} questions via API...`);
  console.log(`API URL: ${BASE_URL}/api/tts`);
  ...
  for (let i = 0; i < packageBListening.length; i++) {
    ...
    try {
      ...
      if (text.length > 1024) {
        ...
        console.log(`  Warning: Text truncated to 1024 chars`);
      }
      ...
      if (data.success && data.audioUrl) {
        ...
        console.log(`  ✓ Saved: ${audioFilename} (${buffer.length} bytes)`);
        successCount++;
      } else {
...
// Run the script
generateAudioViaAPI().catch(console.error);
