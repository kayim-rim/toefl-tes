...
import { ArrowLeft, Headphones, ChevronRight, CheckCircle, Circle, PlayCircle, Loader2 } from 'lucide-react';
import { listeningSkills } from '@/data/learning/skills';
...
export default function ListeningSectionPage() {
  ...
  const fetchProgress = async () => {
    try {
      const response = await fetch('/api/learn/progress?section=listening');
      if (response.ok) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">Listening Skills</h1>
                <p className="text-xs text-slate-400">6 Skills • 60 Questions</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {listeningSkills.map((skill) => {
            ...
            return (
              <Link key={skill.id} href={`/learn/skill/${skill.skillId}`}>
                <Card className="bg-slate-800/80 border-slate-700 hover:border-blue-500/50 transition-all cursor-pointer">
