...
import { ArrowLeft, PenTool, ChevronRight, CheckCircle, Circle, PlayCircle, Loader2 } from 'lucide-react';
import { structureSkills } from '@/data/learning/skills';
...
export default function StructureSectionPage() {
  ...
  const fetchProgress = async () => {
    try {
      const response = await fetch('/api/learn/progress?section=structure');
      if (response.ok) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">Structure Skills</h1>
                <p className="text-xs text-slate-400">12 Skills • 120 Questions</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {structureSkills.map((skill) => {
            ...
            return (
              <Link key={skill.id} href={`/learn/skill/${skill.skillId}`}>
                <Card className="bg-slate-800/80 border-slate-700 hover:border-emerald-500/50 transition-all cursor-pointer">
