...
import {
  ...
  Headphones, PenTool, BookOpen, BarChart3, Award, Users, Loader2, CreditCard
} from 'lucide-react';
...
export default function PricingContent() {
  ...
  // Check for payment finish redirect
  useEffect(() => {
    if (searchParams.get('payment') === 'finish') {
      // Refresh user data to check new tier
      fetchUser();
    }
  }, [searchParams]);
  ...
  const handlePayment = async (tier: 'tes' | 'student') => {
    if (!user) {
    ...
    if (!isSnapLoaded) {
      alert('Payment system sedang dimuat. Silakan tunggu sebentar.');
      return;
    }
    ...
    try {
      ...
      window.snap.pay(data.token, {
        onSuccess: (result) => {
          ...
          // Refresh user data
          fetchUser();
          router.push('/learn?payment=success');
        },
        onPending: (result) => {
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {pricingPlans.map((plan) => {
            ...
            const isPaying = paymentLoading === plan.tier;
              ...
              >
                ...
                <CardFooter>
                  <Button
                    ...
                    disabled={isCurrentPlan || isPaying || (plan.tier !== 'free' && paymentLoading !== null)}
                  >
                    {isPaying ? (
                      <>
          ...
          <div className="space-y-4">
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              ...
              <p className="text-slate-400 text-sm">
                Login ke akun Anda, pilih paket yang diinginkan, lalu lakukan pembayaran melalui
                berbagai metode pembayaran yang tersedia (Transfer Bank, E-Wallet, dll).
              </p>
            </div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              ...
              <p className="text-slate-400 text-sm">
                Semua paket berbayar (Tes dan Student) berlaku selama 1 tahun sejak tanggal pembayaran berhasil.
              </p>
            </div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              ...
              <p className="text-slate-400 text-sm">
                Kami mendukung berbagai metode pembayaran: Transfer Bank (BCA, Mandiri, BNI, BRI, dll),
                E-Wallet (GoPay, OVO, Dana, ShopeePay), QRIS, dan Kartu Kredit/Debit.
              </p>
            </div>

            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
              <h4 className="text-white font-medium mb-2">Apakah pembayaran aman?</h4>
              <p className="text-slate-400 text-sm">
                Ya, semua pembayaran diproses melalui Midtrans, payment gateway resmi yang terdaftar
